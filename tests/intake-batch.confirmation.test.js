...
const { saveManualDraftBatch, markBatchPendingReview, confirmIntakeBatch, confirmManualBatchFromInput, getManualDraftBatch } = require('../src/services/intake-batch-service');
const { getActiveInventoryForDisplay, getConfirmedInventoryItem, createConfirmedInventoryItem } = require('../src/services/inventory-service');
...
test('confirmManualBatchFromInput saves and confirms rows in one step without a review detour', async () => {
  await resetTables();
  ...
  assert.equal(confirmation.state, 'confirmed');
  assert.equal(confirmation.createdItems.length, 2);
  assert.equal(confirmation.createdItems[0].source_batch_id, confirmation.batchId);

  const activeItems = await getActiveInventoryForDisplay();
  assert.deepEqual(activeItems.map((item) => item.name), ['Milk', 'Rice']);

  const batch = await getManualDraftBatch(confirmation.batchId);
  assert.equal(batch.state, 'confirmed');
  assert.equal(batch.rows.length, 2);
});

test('confirmManualBatchFromInput rejects an empty batch and creates no inventory', async () => {
  await resetTables();
  ...
  await assert.rejects(
    () => confirmManualBatchFromInput({
      batchId: null,
      rows: [
        { name: '', quantity: '', unit: '', location: '', expirationDate: '', dateType: '', accepted: true }
      ]
    }),
    (error) => {
      assert.equal(error.code, 'VALIDATION_FAILED');
      assert.match(error.details.join(' '), /At least one included row/);
      return true;
    }
  );
...
test('confirmManualBatchFromInput reports row validation details and stores nothing', async () => {
  await resetTables();
  ...
  await assert.rejects(
    () => confirmManualBatchFromInput({
      batchId: null,
      rows: [
        { name: 'Milk', quantity: '-1', unit: 'package', location: 'fridge', expirationDate: '2026-02-30', dateType: 'best_before', accepted: true }
      ]
    }),
    (error) => {
      assert.equal(error.code, 'VALIDATION_FAILED');
...
test('confirmManualBatchFromInput rolls back the whole transaction when a later insert fails', async () => {
  await resetTables();
  ...
  await assert.rejects(
    () => confirmManualBatchFromInput({
      ...
      inventoryWriter: async (item, client) => {
        ...
        const created = await createConfirmedInventoryItem(item, client);
        if (callCount === 2) {
          throw new Error('DIRECT_SAVE_INSERT_FAILURE_AFTER_FIRST_SUCCESS');
        }
        return created;
      }
    }),
    /DIRECT_SAVE_INSERT_FAILURE_AFTER_FIRST_SUCCESS/
  );
